...
const Addproducts = () => {
  ...
  function validateForm(data) {
    ...
    for (let index in data) {
        ...
        if (!item.fullDescription) {
            missingFields.push('Full Description');
        }
        if (!item.manufacturerDrop) {
        ...
        if (!item.sgstNumber) {
            missingFields.push('SGST Number');
        }
        if (!item.Weight || isNaN(item.Weight)) {
          missingFields.push('Weight (must be a number)');
      }
      //addying dynamic attributes
        if (missingFields.length > 0) {
            isValid = false; 
            toast.error(`Missing fields for index ${index}: ${missingFields.join(', ')}`);
        }
    }
  ...
  const handleProductAdd = async () => {
    setLoading(true);
    console.log("hello",globalformData)
    const isValid = validateForm(globalformData);
    if (!isValid) {
        return
    }
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_ENDPOINT}/api/add_products/add/${sellerId}`,
        {
  ...
  const handleRemoveImage = (index) => {
    ...
    setUploadedImages((prevData) => {
      if (Array.isArray(prevData)) {
        return prevData.filter((_, imgIndex) => imgIndex !== index);
      }
    ...
    setglobalFormData((prevData) => {
      ...
      if (newData?.[index]?.Images?.[trackImageIndex]) {
        newData[index].Images = newData[trackImageIndex].Images.filter(
          ...
          <div className="font-bold pl-1 m-5 flex text-lg justify-start items-start flex-row ">
            ...
            >
              <path
                ...
                fill="#27272A"
              />
            </svg><span className="font-bold">Products</span><RiArrowRightSLine className="text-center justify-center items-center w-7 h-7" />
            <button className="text-[13px] text-gray-400 ml-2">Add Products</button>
          </div>
             ...
             {images.map((image, index) => (
               ...
               >
                 <img
                  ...
                  src={image}
                   alt="Uploaded image"
                   className="rounded-lg object-contain cursor-pointer hover:scale-105 transition-transform duration-300 w-24 h-24"
